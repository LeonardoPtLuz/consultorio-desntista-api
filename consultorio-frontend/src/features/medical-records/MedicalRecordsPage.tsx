...
import { Plus, X, ChevronLeft, ChevronRight, FileText, Edit2, Trash2 } from 'lucide-react';
...
export default function MedicalRecordsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<PageResponse<MedicalRecord>>(
        `/api/medical-records/patient/${searchPatientId}`,
        { params: { page, size: 15 } }
      );
  ...
  // Carregar pacientes, dentistas e tratamentos
  useEffect(() => {
    api.get('/api/patients', { params: { size: 200 } })
      .then(r => setPatients(r.data.content || r.data))
      .catch(() => {});

    api.get<Dentist[]>('/api/dentists')
      .then(r => setDentists(r.data))
      .catch(() => {});

    api.get<Treatment[]>('/api/treatments')
  ...
  const openEditModal = (record: MedicalRecord) => {
    setForm({
      ...
      priceCharged: record.priceCharged ? String(record.priceCharged) : '',
    });
  ...
  const handleSave = async () => {
    if (!form.patientId || !form.dentistId || !form.description.trim()) {
      setError("Paciente, Dentista e Descrição são obrigatórios");
      return;
    }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(e.response?.data?.message || 'Erro ao excluir o prontuário');
    }
  };
        ...
        <div className="space-y-4">
              ...
              >
                  ...
                  {r.priceCharged && (
                    <div className="text-right ml-6">
                      <p className="text-emerald-400 font-semibold">
                        R$ {Number(r.priceCharged).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 text-sm bg-zinc-800 hover:bg-zinc-700 rounded-xl transition-colors text-white"
                  >
                    <Edit2 size={16} /> Editar
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleDelete(r.id); }}
                    className="flex items-center gap-1.5 px-4 py-2 text-sm bg-red-950 hover:bg-red-900 text-red-400 rounded-xl transition-colors"
                  >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-sm">
              <span className="text-zinc-400">Página {page + 1} de {totalPages}</span>
              <div className="flex gap-2">
                <button
                  onClick={() => setPage(p => Math.max(0, p - 1))}
                  disabled={page === 0}
                  className="p-2 text-zinc-400 hover:text-white disabled:opacity-30 rounded-lg bg-zinc-900 border border-zinc-800 transition-colors"
                >
                  <ChevronLeft size={18} />
                </button>
                <button
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-zinc-800 sticky top-0 bg-zinc-900">
              <h2 className="text-xl font-bold text-white">
                {isEditing ? `Editar Prontuário #${editingId}` : 'Novo Prontuário'}
              </h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FI label="Paciente *">
                  <select value={form.patientId} onChange={f('patientId')} className={inp} disabled={isEditing}>
                    <option value="">Selecionar...</option>
                    {patients.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </FI>
                <FI label="Dentista *">
                  <select value={form.dentistId} onChange={f('dentistId')} className={inp} disabled={isEditing}>
                    <option value="">Selecionar...</option>
                ...
                <FI label="Número do dente">
                  <input value={form.toothNumber} onChange={f('toothNumber')} className={inp} placeholder="Ex: 36" />
                </FI>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {saving ? 'Salvando...' : isEditing ? 'Atualizar Prontuário' : 'Salvar Prontuário'}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-zinc-800 sticky top-0 bg-zinc-900">
              ...
              <button onClick={() => setViewing(null)} className="p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-800">
                <X size={20} />
              </button>
            </div>
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                {viewing.priceCharged && (
                  <VF
                    label="Valor cobrado"
                    value={`R$ ${Number(viewing.priceCharged).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`}
                  />
                )}
              </div>
